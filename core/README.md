...
| `outbox.py` | stage-first send discipline and the ONLY caller of `adapter.send()`: durable INTENT → send → read-back VERIFY → COMMIT, so a crash at any seam recovers to exactly-once; staged targets stop at the operator gate; sends paced at ≤1 message/second per channel; reply policy resolvable per placement (thread vs channel, each deny-by-default) with the placement recorded on the row so recovery never resurfaces a thread reply top-level |
